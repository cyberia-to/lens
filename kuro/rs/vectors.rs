...
#[test] fn f2_add_00() { assert_eq!(F2(0).add(F2(0)), F2::ZERO); }
#[test] fn f2_add_01() { assert_eq!(F2(0).add(F2(1)), F2::ONE); }
#[test] fn f2_add_10() { assert_eq!(F2(1).add(F2(0)), F2::ONE); }
#[test] fn f2_add_11() { assert_eq!(F2(1).add(F2(1)), F2::ZERO); }
#[test] fn f2_mul_00() { assert_eq!(F2(0).mul(F2(0)), F2::ZERO); }
#[test] fn f2_mul_01() { assert_eq!(F2(0).mul(F2(1)), F2::ZERO); }
#[test] fn f2_mul_11() { assert_eq!(F2(1).mul(F2(1)), F2::ONE); }
#[test] fn f2_inv()    { assert_eq!(F2(1).inv(), F2::ONE); }
#[test] fn f2_square() { assert_eq!(F2(1).square(), F2::ONE); }

// ===== F₂² =====

#[test] fn f2_2_add_self_zero() {
    for i in 0..4u8 {
        let a = F2_2(i);
...
#[test] fn f2_2_mul_zero() {
    for i in 0..4u8 {
        assert_eq!(F2_2(i).mul(F2_2::ZERO), F2_2::ZERO, "a * 0 = 0");
    }
}

#[test] fn f2_2_mul_specific() {
...
#[test] fn f2_8_mul_identity() {
    let a = F2_8(0xAB);
    assert_eq!(a.mul(F2_8::ONE), a);
}

#[test] fn f2_8_add_self() {
    let a = F2_8(0xFF);
    assert_eq!(a.add(a), F2_8::ZERO);
}

#[test] fn f2_8_inv_roundtrip() {
...
#[test] fn f2_16_mul_identity() {
    let a = F2_16(0xABCD);
    assert_eq!(a.mul(F2_16::ONE), a);
}

#[test] fn f2_16_inv_roundtrip() {
...
#[test] fn f2_16_sqrt_roundtrip() {
    let a = F2_16(0xBEEF);
    assert_eq!(a.sqrt().square(), a);
}

#[test] fn f2_16_square_linearity() {
    let a = F2_16(0x1234);
    let b = F2_16(0x5678);
    assert_eq!(a.add(b).square(), a.square().add(b.square()));
}
...
#[test] fn f2_32_mul_identity() {
    let a = F2_32(0xDEADBEEF);
    assert_eq!(a.mul(F2_32::ONE), a);
}

#[test] fn f2_32_inv_roundtrip() {
...
#[test] fn f2_32_sqrt_roundtrip() {
    let a = F2_32(0xDEADBEEF);
    assert_eq!(a.sqrt().square(), a);
}

#[test] fn f2_32_mul_commutative() {
    let a = F2_32(0x12345678);
    let b = F2_32(0x9ABCDEF0);
    assert_eq!(a.mul(b), b.mul(a));
}

#[test] fn f2_32_mul_associative() {
    let a = F2_32(0x11);
    let b = F2_32(0x22);
    let c = F2_32(0x33);
...
#[test] fn f2_32_mul_distributive() {
    let a = F2_32(0xAA);
    let b = F2_32(0xBB);
    let c = F2_32(0xCC);
...
#[test] fn f2_64_frobenius_period() {
    let a = F2_64(0xAB);
    assert_eq!(a.frobenius(64), a);
}

#[test] fn f2_64_square_linearity() {
...
#[test] fn f2_128_mul_distributive() {
    let a = F2_128(0xAA);
    let b = F2_128(0xBB);
    let c = F2_128(0xCC);
    assert_eq!(a.mul(b.add(c)), a.mul(b).add(a.mul(c)));
...
#[test] fn packed_add_xor() {
    let a = Packed128(0xFF);
    let b = Packed128(0x0F);
    assert_eq!(a.add(b), Packed128(0xF0));
}

#[test] fn packed_mul_and() {
    let a = Packed128(0xFF);
    let b = Packed128(0x0F);
    assert_eq!(a.mul(b), Packed128(0x0F));
}

#[test] fn packed_self_inverse() {
    let a = Packed128(0xDEAD_BEEF_CAFE_BABE_1234_5678_9ABC_DEF0);
    assert_eq!(a.add(a), Packed128::ZERO);
}

#[test] fn packed_inner_product() {
    let a = Packed128(0b1111_0000);
    let b = Packed128(0b1010_1010);
    assert_eq!(a.inner_product(b), 2);
}

#[test] fn packed_popcount_zero() { assert_eq!(Packed128(0).popcount(), 0); }
#[test] fn packed_popcount_one()  { assert_eq!(Packed128(1).popcount(), 1); }
#[test] fn packed_popcount_max()  { assert_eq!(Packed128(u128::MAX).popcount(), 128); }
#[test] fn packed_popcount_half() {
    assert_eq!(Packed128(0xAAAA_AAAA_AAAA_AAAA_AAAA_AAAA_AAAA_AAAA).popcount(), 64);
}
...
#[test] fn encode_decode_128_roundtrip() {
...
#[test] fn encode_decode_64_roundtrip() {
...
#[test] fn encode_decode_32_roundtrip() {
